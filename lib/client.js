...
class Client {
  ...
  async request(method, path, query, body, headers = {}, isRawBuf = false) {
    ...
    var responseBody
    if (!isRawBuf || response.headers['x-fc-error-type']) {
   ...
   * @return {Promise} 返回 Response
   */
  post(path, body, headers, queries, isRawBuf = false) {
    return this.request('POST', path, queries, body, headers, isRawBuf);
  }
   ...
   */
  invokeFunction(serviceName, functionName, event, headers = {}, qualifier, isRawBuf = false ) {
    ...
    return this.post(path, event, headers, null, isRawBuf);
  }
